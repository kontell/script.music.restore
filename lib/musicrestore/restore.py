...
The files go onto the playlist in one ``Playlist.Add``. Building a
``ListItem`` per track in Python is what made a long queue sit there on a
slow device: each call into Kodi drops and retakes the interpreter lock.
The stored song id is checked with ``GetSongDetails`` for the track that is
starting, and a path scan only when that id now names a different song. Once
playback is going, the rest of the queue is checked the same way, from the
next track through to the end and then from the top up to the one that
started, and that is when those items get their tags and art.
"""
...
# Playlist.Add answers before the app thread has inserted the items.
ADD_TIMEOUT = 120.0
...
def _same_queued_file(queued: str, recorded: str) -> bool:
    """True when a queued path is still the recorded one.

    A library hit can rewrite a Jellyfin URL without changing the item id.
    """
    queued_id = rebind.jellyfin_audio_id(queued)
    recorded_id = rebind.jellyfin_audio_id(recorded)
    if queued_id or recorded_id:
        return queued_id == recorded_id
    return queued == recorded


def _first_order_mismatch(
    playlist: xbmc.PlayList, tracks: List[Track]
) -> Optional[int]:
    try:
        if int(playlist.size()) < len(tracks):
            return 0
    ...
    for index, track in enumerate(tracks):
        try:
            ...
            return index
        if not _same_queued_file(item.getPath() or "", track.file):
            return index
    return None


def _queue_files(playlist: xbmc.PlayList, files: List[str]) -> bool:
    """Put every file on the music playlist in one call.

    ``Playlist.Add`` of an array of song ids sorts the list by track number.
    An array of file paths does not: after the first file,
    ``CFileItemList::Sort`` sees the list is already sorted by track number
    and returns, so the rest stay in the order they were given. The call
    answers before the app thread has inserted anything, so wait for the
    length.
    """
    playlist.clear()
    queued = jsonrpc.call(
        ...
        item=[{"file": path} for path in files],
    )
    if queued is None:
        log.error("Playlist.Add failed")
        return False
    deadline = time.monotonic() + ADD_TIMEOUT
    while int(playlist.size()) < len(files):
        if time.monotonic() >= deadline:
            log.error("playlist stopped at %d of %d", playlist.size(), len(files))
            return False
...
def restore(
    ...
    if not _queue_files(playlist, [track.file for track in tracks]):
        return False
    mismatch = _first_order_mismatch(playlist, tracks)
    if mismatch is not None:
        log.error("playlist order differs from the record at %d", mismatch)
    ...
    resume_jelly = rebind.jellyfin_audio_id(resume.file) is not None
    _fill(
        current,
        resume,
        resume_art,
        bool(resume.songid),
        resume_jelly and not resume_settled,
