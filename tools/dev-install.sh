...
DEST="$HOME/.kodi/addons/script.music.restore"
...
sleep 2
# The service is what records queues, so a reinstall has to bounce it —
...
for state in false true; do
    curl -s -u kodi:kodi -X POST -H 'Content-Type: application/json' \
        -d "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"Addons.SetAddonEnabled\",\"params\":{\"addonid\":\"script.music.restore\",\"enabled\":$state}}" \
        "$KODI_RPC" > /dev/null
    sleep 2
done
echo "installed and enabled script.music.restore"
